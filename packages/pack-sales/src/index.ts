...
export function register(api: OpenClawPluginApi) {
  api.registerCommand({
    ...
    handler: async () => ({
      text: "Use the sales-call-summary skill to process the call notes or transcript provided by the user.",
    }),
  });
  ...
  api.registerCommand({
    ...
    handler: async () => ({
      text: "Use the sales-forecast skill to build a forecast. Ask the user for the time period.",
    }),
  });
  ...
  api.registerCommand({
    ...
    handler: async () => ({
      text: "Use the sales-pipeline-review skill to analyze the pipeline. Ask the user for their pipeline data.",
    }),
  });
}
